...
| Area | Shipped |
| --- | --- |
| Connections | PostgreSQL, MySQL/MariaDB, SQLite · SSH tunnels · SSL modes · user/password, pgpass, AWS IAM (RDS/Aurora), no-auth · read-only sources (enforced server-side) · session time zone per source (Local by default) · env color labels · Project (workspace, the default) and Global (user) scopes · passwords only in the OS keychain |
...
| Results | Tablecloth panel shaped like IntelliJ's Services window: Database → source → console tree, per-console result tabs and Output logs, a data source Information tab · multi-statement runs, one tab per query · Explain Plan and Explain Analyse in a Plan tab: the operations as a tree or a table with costs, row estimates, and after analysing the actual rows and time per node |
...
**Docs.** The site at [tablecloth.kenroy.dev](https://tablecloth.kenroy.dev/) is built by GitHub Pages from the `gh-pages` branch; edit the Markdown there. The roadmap page is generated, see `_tools/README.md` on that branch.
